...
export const ExamActiveView: React.FC<ExamActiveViewProps> = ({
  ...
  const [secondsRemaining, setSecondsRemaining] = useState<number>(() => {
    const end = new Date(attempt.expectedEndAt).getTime();
        ...
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="text-sm sm:text-base font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                ...
                <span className="text-xs text-slate-400 font-normal">من {totalCount}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold text-xs hover:bg-slate-800 dark:hover:bg-slate-100 transition-all cursor-pointer shadow-sm"
            >
              تسليم
            </button>
          </div>
      ...
      <main className="max-w-3xl w-full mx-auto px-4 py-6 sm:py-8 flex-1 flex flex-col justify-between">
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80">
                  {!showHint ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 text-xs font-semibold text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 transition-colors py-1 cursor-pointer"
                    >
                      <Lightbulb className="w-3.5 h-3.5 text-amber-500" />
                      <span>عرض تلميح السؤال الفيزيائي</span>
                    </button>
                  ) : (
                    <div className="p-3.5 rounded-2xl bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200/60 dark:border-amber-900/50 space-y-1">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5 text-xs font-bold text-amber-900 dark:text-amber-300">
                          <Lightbulb className="w-4 h-4 text-amber-500" />
                          <span>تلميح وإرشاد تعليمي:</span>
                        </div>
                        <button
                          type="button"
                          onClick={() => setShowHint(false)}
                          className="text-[11px] text-amber-700/80 dark:text-amber-400/80 hover:text-amber-900 dark:hover:text-amber-200 cursor-pointer"
                        >
                          إخفاء التلميح
                        </button>
                      </div>
                      <p className="text-xs text-amber-950 dark:text-amber-100 leading-relaxed pr-5">
                        {currentQuestion.hint}
                      </p>
                    </div>
                  )}
        ...
        <div className="mt-8 pt-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className={`px-5 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 font-bold text-xs sm:text-sm flex items-center gap-1.5 transition-all ${
              currentIndex === 0
          ...
          {currentIndex < totalCount - 1 ? (
            <button
              ...
              className="px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 active:scale-95 text-white font-bold text-xs sm:text-sm flex items-center gap-1.5 shadow-md shadow-blue-500/20 transition-all cursor-pointer"
            >
              <span>التالي</span>
            ...
            <button
              ...
              onClick={() => setShowSubmitModal(true)}
              disabled={isSubmitting}
              className="px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white font-bold text-xs sm:text-sm flex items-center gap-2 shadow-md shadow-emerald-500/20 transition-all cursor-pointer"
            >
